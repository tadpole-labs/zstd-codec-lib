...
validate(await decompressSync(compressedWithDict, undefined, { dictionary: dict }), 'decompressSync (with dict)');
validate((await decompressStream(compressedWithDict, true, { dictionary: dict })).buf, 'decompressStream (with dict)');
validate(await readStream(compressedWithDict, dict), 'ZstdDecompressionStream (with dict)');

validate(await decompressSync(compressedNoDict), 'decompressSync (no dict)');
...
const wasmModule = new WebAssembly.Module(readFileSync(new URL('../../packages/zstd-wasm-decoder/src/_esm/zstd-decoder-perf.wasm', import.meta.url)));

let decoderWithDict = new ZstdDecoder({ dictionary: dict });
decoderWithDict.init(wasmModule);

validate(decoderWithDict.decompressSync(compressedWithDict), 'ZstdDecoder instance (decompressSync with dict)');
validate(decoderWithDict.decompressStream(compressedWithDict, true).buf, 'ZstdDecoder instance (decompressStream with dict)');
validate(decoderWithDict.decompressSync(compressedNoDict), 'ZstdDecoder instance (decompressSync no dict)');
validate(decoderWithDict.decompressStream(compressedNoDict, true).buf, 'ZstdDecoder instance (decompressStream no dict)');
